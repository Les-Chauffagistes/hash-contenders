...
export async function getBattleStatus(battleId: number | string, includeHits: boolean = false): Promise<BattleStatus> {
    return await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/status/${battleId}${includeHits ? "?includes=hits" : ""}`).then(data => data.json());
}

export async function getBattleHits(battleId: number | string): Promise<Round[]> {
    return await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/hits/${battleId}`).then(data => data.json());
}

export async function getAllBattles(): Promise<Battle[]> {
    return await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/battles`).then(data => data.json());
}

export async function createBattle(battle: CreateBattle): Promise<Battle> {
    return await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/battle`, {
        method: "POST",
...
export async function getMe(): Promise<components["schemas"]["User"] | null> {
    const url = `${process.env.NEXT_PUBLIC_AUTH_API_URL}/me`;
    console.log(url)
