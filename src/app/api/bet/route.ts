...
export const POST = withRequestLogging(async (request: Request) => {
    // withRequestLogging a déjà posé un span SERVER comme contexte actif :
    // capturé ici pour être réinjecté explicitement autour des appels sortants
    // après la transaction, plus loin dans submitBet (voir create.ts).
    const parentCtx = activeTraceContext();
    ...
    try {
        await submitBet(prisma, bet.data, access_token, parentCtx);
    } catch (e) {
